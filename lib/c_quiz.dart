...
class Quiz_c {
  ...
  var questions = [
    "1-  How to round-off a value “5.77” to 6.0?",
    "2-  What function can be used to free the memory allocated by calloc()?",
    "3-  To print a double value which format specifier can be used?",
    "4-  Which of the following statement can be used to free the allocated memory?",
  ];

  var choices = [
    ["ceil(5.77)", "round-off(5.77)", "round-up(5.77)", "floor(5.77)"],
    ["dealloc()", "strcat()", "free()", "memcpy()"],
    ["%L", "%lf", "%Lf", "None of the above"],
    ["remove(var-name)", "free(var-name)", "vanish(var-name)", "erase(var-name)"]
  ];

  var correctAnswers = ["ceil(5.77)", "free()", "%lf", "free(var-name)"];
}

class c_quiz extends StatefulWidget {
  final int questionNumber,finalScore;
  c_quiz({Key key, this.questionNumber, this.finalScore}) : super(key: key);
...
class Quiz1State extends State<c_quiz> {
